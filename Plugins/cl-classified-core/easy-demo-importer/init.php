...
if ( ! defined( 'CL_CLASSIFIED_CORE_DEMO_IMPORTER_PATH' ) ) {
	define( 'CL_CLASSIFIED_CORE_DEMO_IMPORTER_PATH', __DIR__ );
}

if ( ! defined( 'CL_CLASSIFIED_CORE_DEMO_IMPORTER_URL' ) ) {
	define( 'CL_CLASSIFIED_CORE_DEMO_IMPORTER_URL', plugin_dir_url( __FILE__ ) );
}
...
spl_autoload_register(
	function ( $class ) {
		$namespaces = [
			'RadiusTheme\\CL_Classified_Core\\' => __DIR__ . '/src/',
		];
...
$demo_config = array_merge(
	[
		...
		'core_base_dir'    => CL_CLASSIFIED_CORE_BASE_DIR,
		'demo_content_dir' => CL_CLASSIFIED_CORE_DEMO_IMPORTER_PATH,
		'demo_content_url' => CL_CLASSIFIED_CORE_DEMO_IMPORTER_URL,
	],
	$demo_config
);
...
try {
	new RadiusTheme\CL_Classified_Core\Core( $demo_config );
} catch ( Exception $e ) {
