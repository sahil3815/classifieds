...
abstract class Custom_Widget_Base extends Widget_Base {
	...
	public function __construct( $data = [], $args = null ) {
		$this->rt_category = CL_CLASSIFIED_CORE_THEME_PREFIX
		                     . '-widgets'; // Category /@dev
	...
	protected function register_controls() {
		...
		foreach ( $fields as $field ) {
			if ( isset( $field['mode'] )
			     && $field['mode'] == 'section_start'
			) {
				$id = $field['id'];
				unset( $field['id'] );
				unset( $field['mode'] );
				$this->start_controls_section( $id, $field );
			} elseif ( isset( $field['mode'] )
			           && $field['mode'] == 'section_end'
			) {
				$this->end_controls_section();
			} elseif ( isset( $field['mode'] )
			           && $field['mode'] == 'tabs_start'
			) {
				$id = $field['id'];
				unset( $field['id'] );
				unset( $field['mode'] );
	...
	public function rt_template( $template, $data ) {
		$template_name = DIRECTORY_SEPARATOR . 'elementor-custom'
		                 . DIRECTORY_SEPARATOR . basename( $this->rt_dir )
