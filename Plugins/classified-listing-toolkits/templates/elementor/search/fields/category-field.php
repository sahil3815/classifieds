...
<?php if ( $settings['category_field'] ) : ?>
    <div class="rtcl-form-group ws-item ws-category ws-category-<?php echo esc_attr( $style ); ?> rtcl-col-sm-6 rtcl-col-12">
        <?php if ( $settings['fields_label'] ) { ?>
            <label for="rtcl-category-search-<?php echo esc_attr( $id ); ?>"><?php esc_html_e( 'Category', 'classified-listing-toolkits' ); ?></label>
        <?php } else { ?>
            <label class="screen-reader-text" for="rtcl-category-search-<?php echo esc_attr( $id ); ?>"><?php esc_html_e( 'Category', 'classified-listing-toolkits' ); ?></label>
        <?php } ?>
        <?php
        if ( $style === 'standard' || $style === 'suggestion' ) {
            ...
            <div class="rtcl-search-input-button rtcl-form-control  rtcl-search-input-category ">
                ...
                <input type="hidden" id="rtcl-category-search-<?php echo esc_attr( $id ); ?>" name="rtcl_category" class="rtcl-term-field" value="<?php echo $selected_category ? esc_attr( $selected_category->slug ) : ''; ?>">
            </div>
        <?php } ?>
    </div>
<?php endif; ?>